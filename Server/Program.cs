...
// ---------- Am I missing any ingredients for a specific meal? ----------
...
app.MapPost("/api/receipt/parse", async (HttpRequest req) =>
{
    ...
    const string prompt =
        ...
        "For each item return its name and quantity (e.g. '2', '500g', '1 bottle'). " +
        "Ignore prices, totals, tax, store name, and dates — only the grocery items matter. " +
        "Return as a JSON array like: [{\"name\":\"chicken breast\",\"quantity\":\"500g\"}, ...]";
...
// Bulk-add parsed receipt items into the Pantry table.
app.MapPost("/api/pantry/bulk-add", async (List<ParsedItem> items, AppDbContext db) =>
{
    ...
    foreach (var item in items)
    {
        ...
        var existing = await db.Pantries.FirstOrDefaultAsync(p =>
            p.UserId == userId && p.IngredientName.ToLower() == item.Name.ToLower());
 
        if (existing != null)
        {
            existing.Unit = item.Quantity ?? existing.Unit;
        }
        else
        {
...
record ParsedItem(string Name, string Quantity);
